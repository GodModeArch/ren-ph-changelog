...
- **Zonal-value pages for barangays that no longer exist under that name are removed.** Some pages stood for barangays that had been renamed or abolished, showing decades-old values for a place that is now served, fresher, under its current name: Juban, Sorsogon had `goroyan` (renamed to Guruyan) and `talinga` (abolished, merged into Cogon) both showing 1997 figures while Guruyan and Cogon carry 2024 values. These pages were never matched to an official barangay code yet were still published. The parser now removes such a page only when its name is absent from the official PSGC barangay list for that town and the town is already covered by a newer schedule, so a real barangay that simply was not matched is kept rather than deleted. 398 stale pages removed nationwide; takes effect on the next data refresh.
