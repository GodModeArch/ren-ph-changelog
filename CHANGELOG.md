...
- `src/proxy.ts` now exports `runtime = 'edge'`. The first deploy after the v2.7.2 `middleware.ts` -> `proxy.ts` rename failed with "Node.js middleware is not currently supported" from `@opennextjs/cloudflare`. Next 16 defaults `proxy.ts` to the Node runtime; OpenNext for Workers only supports Edge. `updateSession` was already Edge-safe (`@supabase/ssr` + `NextResponse.next()` + `request.cookies`), so the runtime opt-in is a one-liner
