...
- **Some barangay pages showed zonal values from an old Department Order even though a newer one already covers that barangay.** When a barangay's name was written differently across Department Orders, the newer schedule sometimes failed to attach to it, so the page kept serving an older, lower value. Two cases are fixed: barangays listed under a combined name such as "Aquino / Imelda" (the newer 2024 value now applies to Bulan's Benigno S. Aquino, which had been showing 1997 values), and numbered town-center barangays written inline as "Barangay 1 (Poblacion)" (Lucena City's eleven Poblacion barangays now show their 2020 values instead of 2010). These corrections take effect on the next zonal data refresh.
