...
- JSON-LD on the guide page: `Article` + `LearningResource` (multi-type, with `learningResourceType: Guide`, `audience`, `educationalLevel: Beginner`), `HowTo` with `totalTime: P90D`, `estimatedCost` MonetaryAmount (₱30-50k), 8 `HowToTool` entries, and 5 anchored `HowToStep` entries; `FAQPage` mirroring the 10 visible FAQs; `BreadcrumbList`. Author has `hasCredential` (EducationalOccupationalCredential) linking the PRC license to the Professional Regulation Commission as `GovernmentOrganization` `recognizedBy`. Article publisher and author use `@id` linking into the existing identity graph (`ren.ph/#organization`, `godmode.ph/#principal`)
