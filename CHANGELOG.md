...
### Fixed
- **Five phantom barangay pages will stop resolving once the next reground lands.** Each served a stale page under an address that does not exist in the Philippine Standard Geographic Code, while the real place lives on at its proper page: `Napico` in Pasig (a sitio inside Barangay Manggahan/Rosario, filed by a 25-year-old schedule as its own heading), the two `INT'L VILLAGE` entries in Las Piñas (decades-old printings of B. F. International Village), `Indl Valley` in Marikina (a 1991 abbreviation of Industrial Valley, which already has its own up-to-date page), and `Barangay 176` in Caloocan (Bagong Silang, which the geographic code recoded to "Barangay 176-A" and whose current values already appear there). Examples of the gap removed: Napico showed ₱3,000 against the real ₱20,000; Marikina's Indl Valley showed ₱2,250 against Industrial Valley's ₱46,000; Caloocan's Barangay 176 showed 2014 values against the 2023 figures on Barangay 176-A. All five now return "not found."

### Internal notes
- Parser-only change in `scripts/agentic-parser/dedup_streets.py`: five `(city_slug, barangay_slug)` tuples added to the curated `KNOWN_BIR_NOT_A_BARANGAY` set (`city-of-pasig/napico`, `city-of-las-pinas/int-l-village`, `city-of-las-pinas/intl-village`, `city-of-marikina/indl-valley`, `city-of-caloocan/barangay-176`), which Pass 4.6 drops for any ungrounded barangay. These slip past both dedup prune passes: Pass 4.5 matches only on sibling *names* (the supersession proof lives in the siblings' street/vicinity fields or, for Caloocan, in the PSGC `old_names` rename that name-matching never consults), the Las Piñas twins carry blank effectivity dates that defeat the pre-2010 year gate, Marikina's `indl-valley` scores jaccard 0.33 against `industrial-valley` (below the 0.5 threshold), and none have a subdivision suffix for Layer 2. The parser set is now a superset of the app-layer `KNOWN_BIR_ZONE_LABEL_FAKES` mask, which is safe (the parser drops before insert). Inert against the live database until the next reground + truncate-import + cache-flush deploy. Five regression tests added to `test_stale_sibling_orphan_fix.py` (suite now 23 pass). A Metro Manila low-value sweep also surfaced the larger "Manila Zone+Barangay" grounding gap (70 stranded modern 2019 records vs 121 grounded barangays still on stale 2004 values) and Malabon grounding gaps; both are real-data grounder fixes deferred to a separate PR, not blocklist drops. Findings: `docs/plans/2026-05-30-pasig-napico-phantom-barangay-findings.md`.
